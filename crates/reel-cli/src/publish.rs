//! `reel template publish`: the paved road from a local template TOML to a
//! registry PR. Validates, previews against the demo cast, scaffolds the
//! pack's `templates/` dir, and — when `gh` is available — opens the
//! index PR without leaving the terminal. Every step degrades to printed
//! instructions rather than blocking on missing tooling.
...
pub fn publish(file: &Path, tags: &[String], no_pr: bool, no_preview: bool) -> Result<()> {
    ...
    let t = template::from_toml(&text, stem)
    ...
    if theme::builtin(&t.theme).is_none() {
        println!(
            "warning: theme `{}` is not builtin — installers who don't have \
             it fall back to the default theme",
            t.theme
        );
    }
    ...
    // 3. Scaffold: the template must live at templates/<name>.toml of a
    // GitHub repo for `add`/`try` raw URLs to resolve.
    ...
    let dest = root.join("templates").join(format!("{name}.toml"));
    let canonical_src = file.canonicalize()?;
    if canonical_src != dest {
        std::fs::create_dir_all(dest.parent().expect("templates dir has a parent"))?;
    ...
    let pack_repo = origin_github_repo(&root).ok_or_else(|| {
        anyhow!(
            "no GitHub `origin` remote found in {} — push the pack repo to \
             GitHub first so raw template URLs resolve",
            root.display()
        )
    })?;
    ...
    if pack_repo == registry::REGISTRY_REPO {
        ...
        let pack_desc = format!("Templates from {pack_repo}");
        std::fs::write(&index_path, registry::upsert_entry(&index, &pack_repo, &pack_desc, entry)?)?;
        println!(
    ...
    if no_pr || !gh_ready() {
        ...
        println!(
            "add this entry to your pack in {} of {} (a PR through the \
             GitHub UI works fine):\n{}",
            registry::INDEX_PATH,
    ...
    open_registry_pr(&pack_repo, &name, entry)
}
...
fn open_registry_pr(pack_repo: &str, name: &str, entry: serde_json::Value) -> Result<()> {
    ...
    let (work_repo, pr_head) = if login == registry_owner {
        (registry::REGISTRY_REPO.to_string(), format!("reel-template-{name}"))
    } else {
        gh(
        ...
        (fork, format!("{login}:reel-template-{name}"))
    };
    let branch = format!("reel-template-{name}");
    ...
    let pack_desc = format!("Templates from {pack_repo}");
    let updated = registry::upsert_entry(&current, pack_repo, &pack_desc, entry)?;
    ...
    gh(
        &[
            ...
            "-f", &format!("message=registry: add {pack_repo}/{name}"),
    ...
    let title = format!("registry: add {pack_repo}/{name}");
    let body = format!(
        "Adds `{name}` from `{pack_repo}` to the template registry.\n\n\
         Try it before merging: `reel template try {pack_repo}/{name}`\n\n\
         Opened by `reel template publish`."
    );
    let pr = gh(
        &[
